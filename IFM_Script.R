...
LogLoss <- rep(1,1);
names(LogLoss)<- c("IFM")
names(Models_AIC)<-c("IFM")
names(Models_BIC)<-c("IFM")
names(ACCcv)<-c("IFM")
names(NumParameters)<-c("IFM")
names(Likelihood)<-c("IFM")
names(BrierScore)<-c("IFM")

Precision<-rep(1,1);
Recall<-rep(1,1);
F1 <- rep(1,1);
names(Precision)<-c("IFM")
names(Recall)<-c("IFM")
names(F1)<-c("IFM")
...
IFMprediction <- ifelse(IFMprobabilities >0.5,1,0)

ModelPredictions <- data.frame(Row=mydata$`Row`,R_IFMpredictedProbabilities=IFMpredictedProbabilities,R_IFMprediction=IFMprediction)
