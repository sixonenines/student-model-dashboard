...
LogLoss <- rep(1,1);
names(LogLoss)<- c("AFM")
names(Models_AIC)<-c("AFM")
names(Models_BIC)<-c("AFM")
names(ACCcv)<-c("AFM")
names(NumParameters)<-c("AFM")
names(Likelihood)<-c("AFM")
names(BrierScore)<-c("AFM")

Precision<-rep(1,1);
Recall<-rep(1,1);
F1 <- rep(1,1);
names(Precision)<-c("AFM")
names(Recall)<-c("AFM")
names(F1)<-c("AFM")
...
AFMprobabilities <- AFMTrainingModel %>% predict(mydata, type = "response")
AFMpredictedProbabilities <- ifelse(AFMprobabilities>0.5, AFMprobabilities, 1-AFMprobabilities)
AFMprediction <- ifelse(AFMprobabilities >0.5,1,0)

ModelPredictions <- data.frame(Row=mydata$`Row`,R_AFMpredictedProbabilities=AFMpredictedProbabilities,R_AFMprediction=AFMprediction)
...
confusion_matrix=table(ACTUAL=testdata$Outcome,PREDICTED=testdata$afmprediction)
